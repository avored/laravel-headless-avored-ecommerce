  ...
  methods: {
      async submitForm() {
          ...
          await this.$apollo.mutate({
                ...
                update: (cache, { data: { userAuth } }) => {
                    localStorage.setItem(AUTH_TOKEN, userAuth.access_token)
                    // window.x = app
                    // console.log(app.$parent.$options.data().headerComponentKey  += 1)
                    app.$router.push({ name: 'home'})
                    app.$emit('forceUpdate')
                },
            })
      }
  }
